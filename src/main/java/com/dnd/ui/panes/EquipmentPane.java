...
public class EquipmentPane extends GridPane {
    ...
    public EquipmentPane(ViewModel character, TabPane mainTabPane) {
        ...
        Runnable updateItems = () -> {
            ...
            for (Item item : character.getItems()) {
                ...
                if (!itemPresent) {
                    ...
                    updateMain.run();
                    character.getMainHand().addListener(_ -> {
                        updateMain.run();
                    });
                    ...
                    Runnable updateOff = () -> {
                        if (type.equals("WEAPON")) {
                            ...
                            if (shield.equals(item) || shield.getNominative().equals("NONE_M")) {
                                off.disableProperty().set(false);
                            } else {
                                off.disableProperty().set(true);
                            }
                    ...
                    updateOff.run();
                    character.getOffHand().addListener(_ -> {
                        updateOff.run();
                    });
        ...
        ObservableList<String> allItemsList = FXCollections.observableArrayList(getTranslatedItems());
    ...
    private String[] getTranslatedItems() {
        return ItemManager.getInstance().getTranslatedItems();
    }
